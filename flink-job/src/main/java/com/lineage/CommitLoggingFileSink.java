 ...
 * into the pre-commit topology. All other behavior delegates to the inner FileSink.
 *
 * The commit log operator observes committable messages and writes a JSON manifest
 * recording (checkpoint_id, s3_key) for each committed file.
 *
...
public class CommitLoggingFileSink<IN>
    ...
    public DataStream<CommittableMessage<FileSinkCommittable>> addPreCommitTopology(
        ...
        // First, let the delegate add its own pre-commit topology (e.g., compaction)
        DataStream<CommittableMessage<FileSinkCommittable>> delegateStream =
                delegate.addPreCommitTopology(committableStream);

        // Then chain our commit log extraction operator
        return delegateStream
                .transform(
                        "CommitLogExtractor",
                        delegateStream.getType(),
                        new CommitLogExtractingOperator(commitLogBasePath))
